...
def update(request):
    ...
    form = MovieSelectForm(movies, request.POST or None)

    if request.method == "GET":
        msg = request.GET.get('msg')
        return render(request, 'ex07/update_movie.html', {
            'form': form,
            'msg': msg
        })
        ...
        if form.is_valid():
            ...
            try:
                movie_to_update = movies.filter(title=movie_title)
                movie_to_update.update(opening_crawl=opening_crawl_new_value)
                url = reverse('update_movie_07')
